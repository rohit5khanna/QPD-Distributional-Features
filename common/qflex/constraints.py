...
def solve_with_bounds(Y: np.ndarray, x_data: np.ndarray, terms: int,
    ...
    depends on cond(Y) not cond(Y^T Y).  cond(Y^T Y) = cond(Y)^2, so BVLS
    stays reliable ~10^7 times further into ill-conditioning than SLSQP.
    Because the objective is a convex quadratic and bounds are simple,
    lsq_linear finds the unique global optimum exactly.
    """
    lb = np.full(terms, -np.inf)
    ...
    result = lsq_linear(Y, x_data, bounds=(lb, ub),
                        method='bvls', tol=1e-14, max_iter=10_000)
    if not result.success:
        raise QFlexError(f"Tail constraint optimization failed: {result.message}")
    return result.x
