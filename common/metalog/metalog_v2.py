...
class Metalog:
    ...
    def _unified_pdf_numerical(self, y):
        ...
        is_scalar = y_array.ndim == 0
        y_flat = np.atleast_1d(y_array)

        # Use adaptive step size based on y value:
        # smaller near boundaries, larger in the middle.
        eps_array = np.where(
            y_flat < 0.1, 1e-10,  # Very small near lower bound
            np.where(
                y_flat > 0.9, 1e-10,  # Very small near upper bound
                1e-8  # Standard step in middle
            )
        )

        # Vectorized derivative with per-point adaptive step sizes
        grad = derivative(y_flat, eps_array)

        # Handle problematic regions with larger step sizes
        bad = (~np.isfinite(grad)) | (np.abs(grad) < 1e-12)
        if np.any(bad):
            grad[bad] = derivative(y_flat[bad], 1e-6)

        bad = (~np.isfinite(grad)) | (np.abs(grad) < 1e-12)
        if np.any(bad):
            grad[bad] = derivative(y_flat[bad], 1e-4)
        ...
        if len(pdf) > 5:
            ...
            blend_factor = np.where(
                y_flat < 0.1, 0.7,  # More smoothing near lower bound
                np.where(
                    y_flat > 0.9, 0.7,  # More smoothing near upper bound
        ...
        if is_scalar:
            return float(pdf[0])
        return pdf.reshape(y_array.shape)
